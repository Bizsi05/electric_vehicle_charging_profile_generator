...
def plot_segment_profiles_one_day(profiles,title):
    ...
    plt.title("Weekday – szegmensenkénti töltési profil (1 nap)")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.show()

def plot_daily_energy_by_segment (yearly_profiles, year, dt_minutes):
...
def plot_mean_daily_profile_by_segment(yearly_profiles, year, dt_minutes):
